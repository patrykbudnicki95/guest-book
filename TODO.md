DONE:

- login
- file upload
- base dashboard
- split dashboard to routes
- render link to the event page on dashboard
- qr code generation
- pdf download generated qr code only
- translations
- main page styles
- login from the homepage

TODO (sorted by priorites):

- jak dodajesz zdjęcie jako gość ustawiasz widoczność public/private
- pdf download with information about the event
- main page content
  - pricing options (maybe display some fast shopping options like in vesti, but I'm not sure about that) -> needs to be reviewed, also discuss what specific plans should include
- event page content
- limitations by selected plan -> works, but discuss what specific plan should include
- giving info about the plan -> works, but discuss what specific plan should include
- download all
- pdf download styles
- cookies/policy
- payments
- qr code + fancy card that they can print/we can print and send them
- games ? terenowa gra z qr kodami z pytaniami o państwa młodych, kto do 12 zbierze najwięcej punktów to coś wygrywa, no i wiesz dajesz kilkanaście pytań, dostajesz qr kody ukrywasz gdzieś tam przy ekspresie do kawy XD
- test mode -> guest can browse example event, see details, update all settings, and entries to the guest book that is assigned to that test event BUT ALL of those changes are saved only on his local browser(so users can override that one event locally, to not store any test data/images on our side) - and user should have a button that allow him to save changes of the event after he will buy an real event)
- basic auth - póki nie ma proda, zeby nie zaczęło pozycjonować
